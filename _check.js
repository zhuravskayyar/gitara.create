...
async function loadSelectedInstruments() {
  ...
  const results = await Promise.all(selected.map(type => loadInstrument(type)));
  updateLegacyInstrumentState();
  ...
  if (loadedCount > 0) {
    const nativeHint = USE_WAF_NATIVE_CHORD_API && selected.length === 1
      ? ' | native chord API on'
      : '';
    setInfoText(`Layers: ${title} (${loadedCount}/${selected.length})${nativeHint}`, true);
  } else {
...
function getPlaybackInstrumentLayers(articulation = {}) {
  const selected = getActiveInstrumentList();
...
function playNote(pitch, time, duration, gainVal, articulation = {}) {
  if (!ctx) return;
  const {
    stringIndex = 0,
    direction = 'down',
  ...
  layers.forEach((type, layerIndex) => {
    ...
    const layerGain = Math.max(
      0,
      Math.min(1, safeGain * (cfg.mixGain || 1) * mixNormalize * (1 - layerIndex * 0.03))
    );
...
function mutedStroke(notes, time = 0, gain = 0.28) {
  if (!ctx) return;
  if (!Array.isArray(notes) || notes.length === 0) return;
...
function playPatternMode(notes, beatDuration, hits, withMute = false, baseVel = velocity, stepMeta = null, chordRoot = null) {
  ...
  for (let i = 0; i < hits; i++) {
    ...
    if (isGhost) {
      mutedStroke(notes, actualT, baseVel * 0.34 * accentVal);
      continue;
    }
...
function applyVoiceLeading(notes, chordName) {
  if (!notes.length) return notes;
  if (!previousVoicingPitches || typeof previousVoicingPitches !== 'object') {
    // Store as a map: stringIndex → pitch
    previousVoicingPitches = {};
    notes.forEach(n => { previousVoicingPitches[n.stringIndex] = n.pitch; });
    return notes;
  }

  const result = notes.map(note => {
    // Match by string, not by array position
    const prevPitch = previousVoicingPitches[note.stringIndex];
  ...
  previousVoicingPitches = {};
  result.forEach(n => { previousVoicingPitches[n.stringIndex] = n.pitch; });
  return result;
}
...
function playChord(chordName, mode, beatDuration, singleStringChoice = null, stepMeta = null) {
  if (!ctx) return;
  ...
  if (meterMode) {
    ...
    playPatternMode(notes, beatDuration, meterMode.hits, meterMode.mute, vel, patternMeta, chordRoot);
    return;
  }

  if (tryPlayChordNativeWebAudioFont(notes, mode, beatDuration, vel, stepMeta)) {
    return;
  }
...
function playChordStrum(chordName, hit, durationSecs, singleStringChoice = null) {
  if (!chordName) return;
  ...
  playChord(chordName, mode, Math.max(0.05, durationSecs), singleStringChoice, stepMeta);
}
